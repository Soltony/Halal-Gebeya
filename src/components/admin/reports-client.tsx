...
export function ReportsClient({ providers }: { providers: LoanProvider[] }) {
    ...
    try {
      ...
      if (allLoans.length > 0) {
        const providerLoanData = allLoans.map((d: any) => ({
          Provider: d.provider,
          "Financing ID": d.loanId,
          [IFB.excelColCustomer]: d.borrowerName,
          "Financing released": d.principalDisbursed,
          "Outstanding financing": d.principalOutstanding,
          "Outstanding daily fee": d.interestOutstanding,
          "Outstanding commission": d.serviceFeeOutstanding,
          "Outstanding late charge": d.penaltyOutstanding,
          "Total Outstanding": d.totalOutstanding,
          Status: d.status,
        }));
        const wsProvider = wb.addWorksheet(IFB.excelSheetPartnerFinancing);
      ...
      if (allCollections.length > 0) {
        const collectionsExportData = allCollections.map((d: any) => ({
          ...
          "Financing amount received": d.principal,
          "Daily fee received": d.interest,
          "Commission received": d.serviceFee,
          "Late charge received": d.penalty,
          "Tax Received": d.tax,
      ...
      const utilizationExportData = providerList
        .map((p) => {
          ...
          return {
            ...
            [IFB.excelColFinancingFacilitiesReleased]: data.portfolioSummary.disbursed,
            [IFB.excelColOutstandingFinancingAmt]: data.portfolioSummary.outstanding,
      ...
      if (allDisbursements.length > 0) {
        const disbExport = allDisbursements.map((r: any) => {
          ...
          return {
            ...
            "Financing amount (MLS)": loanAmt,
            "Daily fee (MLS)": interestFee,
            "Commission (MLS)": serviceFee,
        ...
        const ws = wb.addWorksheet(IFB.excelSheetFinancingReleases);
      ...
      providerList.forEach((p) => {
        ...
        borrowers.forEach((b: any) => {
          borrowerAgingExport.push({
            Provider: p.name,
            [IFB.excelColCustomer]: b.borrowerId,
            [`${IFB.customer} account`]: b.borrowerAccount || "",
            [`${IFB.customer} name`]: b.borrowerName || "",
            "Days Overdue": b.daysOverdue ?? "",
            Category: b.classification || "",
            "Outstanding financing": b.principalOutstanding || 0,
            "Outstanding profit": b.interestOutstanding || 0,
            "Outstanding administration fee": b.serviceFeeOutstanding || 0,
            "Outstanding late charge": b.penaltyOutstanding || 0,
            "Total Outstanding": b.classificationAmount || b.totalOverdue || 0,
          });
        });
      });
      if (borrowerAgingExport.length > 0) {
        const wsB = wb.addWorksheet(IFB.excelSheetCustomerAging);
      ...
      if (allLoans.length > 0) {
        const borrowerPerfData = allLoans.map((d: any) => ({
          [`${IFB.customer} ID`]: d.borrowerId,
          [`${IFB.customer} name`]: d.borrowerName,
          "Financing ID": d.loanId,
          "Financing released": d.principalDisbursed,
          "Outstanding financing": d.principalOutstanding,
          "Outstanding profit": d.interestOutstanding,
          "Outstanding administration fee": d.serviceFeeOutstanding,
          "Outstanding late charge": d.penaltyOutstanding,
          "Days in Arrears": d.daysInArrears,
          Status: d.status,
        }));
        const wsBorrower = wb.addWorksheet(IFB.excelSheetCustomerPerformance);
        ...
        <Card className="mt-4">
          ...
          <CardContent>
            <p className="text-muted-foreground">
              You are not currently associated with a financing partner. Please
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      >
        <TabsList>
          <TabsTrigger value="providerReport">Partner financing</TabsTrigger>
          <TabsTrigger value="disbursementsReport">Financing releases</TabsTrigger>
          <TabsTrigger value="repaymentsReport">Settlements</TabsTrigger>
                ...
                <TableRow>
                  ...
                  <TableHead>
                    <button
                      ...
                      className="flex items-center"
                    >
                      Financing ID{renderSortIcon("disbursementsReport", "loanId")}
                    </button>
                  </TableHead>
                  ...
                  <TableHead className="text-right">
                    <button
                      ...
                      className="flex items-center"
                    >
                      {IFB.disbursementColFinancingAmountMls}
                      {renderSortIcon(
                        "disbursementsReport",
                        "principalDisbursed"
                      )}
                  ...
                  <TableHead className="text-right">
                    {IFB.disbursementColMurabahaProfitMls}
                  </TableHead>
                  <TableHead className="text-right">
                    Commission (MLS)
                  </TableHead>
                  <TableHead className="text-right">
                    <button
                      ...
                      className="flex items-center"
                    >
                      {IFB.disbursementColNetReleasedMls}
                ...
                <TableRow>
                  ...
                  <TableHead>
                    <button
                      ...
                      className="flex items-center"
                    >
                      Financing ID{renderSortIcon("repaymentsReport", "loanId")}
                    </button>
                  </TableHead>
                  <TableHead>Customer</TableHead>
                  <TableHead>
                  ...
                  <TableHead className="text-right">Financing amount paid</TableHead>
                  <TableHead className="text-right">Daily fee paid</TableHead>
                  <TableHead className="text-right">Commission paid</TableHead>
                  <TableHead className="text-right">Late charge paid</TableHead>
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-card z-10">
                <TableRow>
                  ...
                  <TableHead className="text-right">Late charge received</TableHead>
                    ...
                    <Table>
                      <TableHeader className="sticky top-0 bg-card z-10">
                        <TableRow>
                          ...
                          <TableHead className="text-right">
                            {IFB.outstandingFinancing}
                          </TableHead>
                          <TableHead className="text-right">
                            Outstanding profit
                          </TableHead>
                          <TableHead className="text-right">
                            Outstanding administration fee
                          </TableHead>
                          <TableHead className="text-right">
                            Outstanding late charge
                          </TableHead>
