...
export function AddProviderDialog({ isOpen, onClose, onSave, provider, primaryColor = BRAND_PRIMARY }: AddProviderDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{provider ? 'Edit partner' : `Add new ${IFB.financingPartner.toLowerCase()}`}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-6 py-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" style={{ backgroundColor: primaryColor }} className="text-white">{provider ? 'Submit for approval' : `Add ${IFB.financingPartner.toLowerCase()}`}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
