...
export async function POST(req: Request) {
  try {
    ...
    // For testing: force the provider id to PRO0003 unless overridden by env
    const forcedProviderId = process.env.FORCE_PROVIDER_ID ?? "PRO0003";
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          if (isMerchantCredit) {
            message = IFB.smsIslamicBnplReleasedToMerchant(String(amt));
          } else {
            message = IFB.smsFinancingReleasedToAccount(String(amt), String(creditAccount));
          }
        } else {
          const reason =
            payload &&
            ...
            (payload.message || payload.Message)
              ...
              ? payload
              : txt ?? "Unknown error";
          message = IFB.smsFinancingReleaseFailed(String(amt));
        }
