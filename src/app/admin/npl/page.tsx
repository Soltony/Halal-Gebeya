...
export default function NplManagementPage() {
    ...
    const fetchBorrowers = async () => {
        ...
        try {
            ...
            toast({
                title: 'Error',
                description: `Could not load ${IFB.npfCustomers.toLowerCase()}.`,
                variant: 'destructive',
            });
        } finally {
            setIsLoading(false);
    ...
    const handleRunNplUpdate = async () => {
        if (!canRunNplUpdate) {
            toast({ title: 'Not authorized', description: `You are not authorized to run ${IFB.npf} updates.`, variant: 'destructive' });
            return;
        }
        setIsUpdating(true);
        try {
            ...
            if (result.success) {
                toast({
                    title: `${IFB.npf} status updated`,
                    description: IFB.npfUpdateCustomersCount(result.updatedCount),
                });
            ...
            toast({
                title: `Error running ${IFB.npf} update`,
        ...
        <>
            <div className="flex-1 space-y-4 p-8 pt-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight">{IFB.npf}</h2>
                        <p className="text-muted-foreground">
                            View and manage {IFB.customers.toLowerCase()} with non-performing financing.
                        </p>
                    </div>
                    {canRunNplUpdate && (
                        <Button onClick={handleRunNplUpdate} disabled={isUpdating}>
                            ...
                            Run {IFB.npf} status update
                        </Button>
                    )}
                </div>
                 <Card>
                    <CardHeader>
                        <CardTitle>{IFB.npfCustomers}</CardTitle>
                        <CardDescription>This list contains all {IFB.customers.toLowerCase()} flagged for overdue financing based on each partner&apos;s {IFB.npf} threshold.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead>{IFB.customer} ID</TableHead>
                                    <TableHead>Status</TableHead>
                                    <TableHead>Overdue facilities</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={3} className="h-24 text-center">
                                            No {IFB.npfCustomers.toLowerCase()} found.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
