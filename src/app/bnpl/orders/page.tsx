    ...
    try {
      ...
      toast({
        ...
        description: isDirectPayment
          ...
          : IFB.orderDeliveredFinancingReleased,
      });
      load();
    } catch (e: any) {
  ...
  const cancelOrder = async () => {
    ...
    try {
      const res = await fetch('/api/bnpl/orders', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: cancelOrderId, status: 'CANCELLED', cancelReason: cancelReason || IFB.cancelOrderByCustomer }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge className={o.paymentType === 'DIRECT' ? 'bg-emerald-100 text-emerald-800 border-emerald-300 text-[10px]' : 'bg-amber-100 text-amber-800 border-amber-300 text-[10px]'} variant="outline">
                    {o.paymentType === 'DIRECT' ? IFB.directPayment : IFB.islamicBnplBadge}
                  </Badge>
                </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground ml-12">{IFB.trackIslamicBnplOrdersSubtitle}</p>
        </div>

        <Card className="mb-6">
          <CardHeader className="pb-2">
            <CardTitle className="text-xl">Orders</CardTitle>
            <p className="text-sm text-muted-foreground">{IFB.customerIdLabelUi}: {borrowerId}</p>
          </CardHeader>
        </Card>
            ...
            {orderStatusTabs.map((tab) => {
              ...
              return (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="relative shrink-0 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600 data-[state=active]:border-secondary data-[state=active]:bg-secondary data-[state=active]:text-secondary-foreground data-[state=active]:text-white"
                >
                  {tab.label} ({count})
                  {notificationCount > 0 && (
