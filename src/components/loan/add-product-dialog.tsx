  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{IFB.addNewFinancingProduct}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
             ...
             <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="min-loan" className="text-right">{IFB.minFinancingAmountLabel}</Label>
              <Input id="min-loan" type="number" value={minLoan} onChange={(e) => setMinLoan(e.target.value)} className="col-span-3" required />
            </div>
             <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="max-loan" className="text-right">{IFB.maxFinancingAmountLabel}</Label>
              <Input id="max-loan" type="number" value={maxLoan} onChange={(e) => setMaxLoan(e.target.value)} className="col-span-3" required />
            </div>
             <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="duration" className="text-right">{IFB.financingDurationDaysLabel}</Label>
              <Input id="duration" type="number" value={duration} onChange={(e) => setDuration(e.target.value)} className="col-span-3" required />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="installments" className="text-right">{IFB.installmentsLabel}</Label>
              <Input id="installments" type="number" value={installments} onChange={(e) => setInstallments(e.target.value)} className="col-span-3" placeholder={IFB.installmentsPlaceholder} />
            </div>
            {installments && Number(installments) > 0 && (
              <div className="grid grid-cols-4 items-center gap-4">
                <Label className="text-right">{IFB.settlementIntervalLabel}</Label>
                <div className="col-span-3 text-sm text-muted-foreground">{IFB.everyNDays(Math.floor((Number(duration) || 0) / Number(installments)) || 0)}</div>
              </div>
            )}
          <DialogFooter>
            <DialogClose asChild>
            ...
            <Button type="submit">{IFB.submitProductForApproval}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
