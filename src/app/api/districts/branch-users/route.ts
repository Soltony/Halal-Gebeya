...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const smsText = IFB.smsWelcomeUserAccount(appUrl, email.trim().toLowerCase(), rawPassword);
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'reset-password') {
      ...
      const smsText = IFB.smsPasswordResetShort(appUrl, targetUser.email, newPassword);
    ...
    if (action === 'resend-sms') {
      ...
      const appUrl =
        ...
        process.env.CALLBACK_URL?.replace(/\/api\/.*$/, '') ||
        'https://nibteraloan.nibbank.com.et';
      const smsText = IFB.smsWelcomeUserAccountWithPassword(appUrl, targetUser.email, newPassword);
      sendSms(targetUser.phoneNumber, smsText).catch((err: any) => {
