...
export default function ReversalApprovalDetailClient({ change, currentUser }: { change: PendingChangeWithDetails; currentUser: User }) {
  ...
  const displaySubtype = isPostedLoan ? IFB.reversalPostedFinancingSuffix : IFB.reversalFailedReleaseSuffix;

  const whatRequestIs = useMemo(() => {
    const who = change.createdBy?.fullName || 'Someone';
    if (isCancel) {
      return `${who} requested to mark this ${isPostedLoan ? IFB.reversalMarkAsSuccessPosted : IFB.reversalMarkAsSuccessFailed} as successful`;
    }
    return `${who} requested to reverse this ${isPostedLoan ? IFB.reversalReverseTargetPosted : IFB.reversalReverseTargetFailed}`;
  }, [change, isCancel, isPostedLoan]);

  const impact = useMemo(() => {
    if (isCancel) {
      return IFB.reversalCancelApproveImpact;
    }
    return IFB.reversalReverseApproveImpact;
  }, [isCancel]);
  ...
  const details = useMemo(() => {
    ...
    if (created.loanId) items.push({ label: IFB.financingIdLabel, value: created.loanId });
    if (created.borrowerId) items.push({ label: IFB.customerIdLabel, value: created.borrowerId });
    ...
    if (created.providerId) items.push({ label: IFB.financingPartner, value: created.providerId });
    if (created.originalProviderId) items.push({ label: 'Original Provider', value: created.originalProviderId });
    if (created.isPosted != null) items.push({ label: IFB.postedFinancingFieldLabel, value: created.isPosted ? 'Yes' : 'No' });
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <Textarea
              ...
              placeholder={IFB.reversalRejectPlaceholderReleaseSucceeded}
              rows={4}
            />
          </div>
          <DialogFooter>
