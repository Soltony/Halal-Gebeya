...
export function HistoryClient({ initialLoanHistory, providers, taxConfigs, asOfDate }: HistoryClientProps) {
  ...
  const [selectedLoanProviderColor, setSelectedLoanProviderColor] = useState<string>(BRAND_PRIMARY);
  ...
  const handleToggleExpand = (loanId: string) => {
      ...
      if (newExpandedLoanId) {
          ...
          setSelectedLoanProviderColor(provider?.colorHex || BRAND_PRIMARY);
      } else {
          // Reset to default or first loan's color when all are collapsed
           ...
           setSelectedLoanProviderColor(firstLoanProvider?.colorHex || BRAND_PRIMARY);
      }
  }
  ...
  const handleConfirmRepayment = async (amount: number) => {
    ...
    try {
      ...
      toast({
        ...
        description: IFB.paymentTowardsFinancing(`${formatCurrency(amount)} ETB`),
      });
  ...
  const renderLoanCard = (loan: LoanDetails) => {
    ...
    const provider = providers.find(p => p.id === loan.product.providerId);
    const color = provider?.colorHex || BRAND_PRIMARY;
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              {loan.repaymentStatus === 'Unpaid' && <Button size="sm" style={{backgroundColor: color}} className="text-white" onClick={() => handleRepay(loan)}>{IFB.settle}</Button>}
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="container py-6 md:py-10">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-2 bg-gray-200">
                        <TabsTrigger value="active">{IFB.activeFinancing}</TabsTrigger>
                        <TabsTrigger value="closed">{IFB.closedFinancing}</TabsTrigger>
                    </TabsList>
                          ...
                          <Card className="shadow-lg text-white transition-colors duration-300" style={{ backgroundColor: selectedLoanProviderColor }}>
                            <CardContent className="p-4 flex justify-around items-center">
                              <div className="text-center">
                                ...
                                <p className="text-xs opacity-90">{IFB.outstandingFinancing} (ETB)</p>
                              </div>
                               <div className="text-center">
                                <p className="text-2xl font-bold">{formatCurrency(totalCreditAmount)}</p>
                                <p className="text-xs opacity-90">{IFB.totalFinancingAmount} (ETB)</p>
                              </div>
                            </CardContent>
                          </Card>
                      ) : (
                          <Card className="shadow-lg text-white transition-colors duration-300" style={{ backgroundColor: selectedLoanProviderColor }}>
                            <CardContent className="p-4 flex justify-around items-center">
                              <div className="text-center">
                                ...
                                <p className="text-xs opacity-90">Total settled (ETB)</p>
                              </div>
                              <div className="text-center">
                                <p className="text-2xl font-bold">{closedLoans.length}</p>
                                <p className="text-xs opacity-90">Total facilities closed</p>
                              </div>
                            </CardContent>
                          </Card>
                      )}
                    ...
                    <TabsContent value="active">
                       <div className="space-y-4">
                           {activeLoans.length > 0 ? activeLoans.map(renderLoanCard) : <p className="text-center text-muted-foreground py-8">No active financing.</p>}
                       </div>
                    </TabsContent>
                    <TabsContent value="closed">
                       <div className="space-y-4">
                           {closedLoans.length > 0 ? closedLoans.map(renderLoanCard) : <p className="text-center text-muted-foreground py-8">No closed financing.</p>}
                        </div>
                    </TabsContent>
                </Tabs>
            </div>
