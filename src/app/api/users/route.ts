...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (roleName === 'Merchant' && password && userData.phoneNumber) {
      ...
      const smsText = IFB.smsWelcomeMerchantAccount(appUrl, userData.email, password);
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'resend-sms') {
      ...
      const smsText = IFB.smsAccountCredentialsReminder(appUrl, targetUser.email);
    ...
    if (action === 'reset-password') {
      ...
      if (targetUser.phoneNumber) {
        const smsText = IFB.smsPasswordResetNotice(appUrl, targetUser.email, newPassword);
        await sendSms(targetUser.phoneNumber, smsText);
      }
