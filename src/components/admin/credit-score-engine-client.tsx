...
export function CreditScoreEngineClient({ initialProviders, initialScoringParameters }: CreditScoreEngineClientProps) {
    ...
    const themeColor = useMemo(() => providers.find(p => p.id === selectedProviderId)?.colorHex || BRAND_PRIMARY, [providers, selectedProviderId]);
    ...
    if (providers.length === 0) {
        return (
            <div className="flex-1 space-y-4 p-8 pt-6">
                <h2 className="text-3xl font-bold tracking-tight">{IFB.riskScoringEngine}</h2>
