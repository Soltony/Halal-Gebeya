...
export async function GET(
  ...
  if (!borrowerId || !providerId) {
    return NextResponse.json({ error: IFB.ussdCustomerAndPartnerRequired }, { status: 400 });
  }
  ...
  try {
    ...
    if (!borrower) {
        return NextResponse.json({ error: IFB.ussdCustomerNotFound }, { status: 404 });
    }
    
    if (!provider) {
        return NextResponse.json({ error: IFB.ussdFinancingPartnerNotFound }, { status: 404 });
    }
