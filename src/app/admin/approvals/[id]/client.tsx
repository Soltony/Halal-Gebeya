...
export default function ApprovalsDetailClient({ change, currentUser }: { change: PendingChangeWithDetails; currentUser: User }) {
  ...
  const metricLabel = (metric: any) => {
    ...
    const map: Record<string, string> = {
      PAID_EARLY: IFB.metricEarlySettlements,
      PAID_LATE: IFB.metricLateSettlements,
      TOTAL_COUNT: IFB.metricTotalFinancingCount,
      PAID_ON_TIME: IFB.metricOnTimeSettlements,
    };
  ...
  const renderLoanCycleConfig = (source: any) => {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>{IFB.financingCycleConfiguration}</CardTitle>
          <CardDescription>{IFB.financingCycleCardDescription}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="text-xs text-muted-foreground mb-1">{IFB.progressionMetricLabel}</div>
              <div className="font-medium">{metricLabel(metric) || '—'}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">{IFB.enabledLabel}</div>
          ...
          {cycleRanges.length > 0 && (
            <div className="space-y-2">
              <div className="font-medium">{IFB.financingCycleRangesSection}</div>
              <div className="border rounded-md overflow-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
          ...
          {grades.length > 0 && cycleRanges.length > 0 && (
            <div className="space-y-2">
              <div className="font-medium">{IFB.financingCycleGradesSection}</div>
              <div className="border rounded-md overflow-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
          ...
          {Array.isArray(legacyCycles) && legacyCycles.length > 0 && (
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">{IFB.legacyCyclesLabel}</div>
    ...
    if (Array.isArray(v)) {
      ...
      const formatPenaltyRuleLocal = (rule: any) => {
        ...
        if (rule.type === 'fixed') {
          ...
          valueString = `${Number.isFinite(Number(raw)) ? Number(raw) : String(raw)}% ${IFB.pctOfFinancingAmount.toLowerCase()}`;
