...
export async function GET(
  ...
  if (!borrowerId) {
    return NextResponse.json({ error: IFB.ussdBorrowerIdRequired }, { status: 400 });
  }
  ...
  try {
    ...
    for (const loan of loans) {
      ...
      transactions.push({
        ...
        description: IFB.ussdFinancingReleaseForProduct(loan.product.name),
      ...
      for (const payment of loan.payments) {
        transactions.push({
          ...
          description: IFB.ussdSettlement,
