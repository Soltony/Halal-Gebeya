...
const ProductSettingsForm = ({ provider, product, providerColor, onSave, onDelete, onUpdate, allDataConfigs }: {
    ...
    const handleFilterFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!canCreateEligibilityList) {
            toast({ title: 'Not authorized', description: IFB.readOnlyEligibility, variant: 'destructive' });
    ...
    const submitForApproval = async (e: React.FormEvent) => {
        ...
        if (!canEditProduct) {
            toast({ title: 'Not authorized', description: IFB.readOnlyProducts, variant: 'destructive' });
            return;
        }
        setIsSaving(true);
        try {
            ...
            if (parsedInstallments !== null) {
                if (!Number.isFinite(parsedInstallments) || parsedInstallments <= 0) {
                    throw new Error(IFB.errorInstallmentsPositive);
                }
                if (parsedDuration <= 0) {
                    throw new Error(IFB.errorFinancingDurationPositive);
                }
                if (parsedInstallments > parsedDuration) {
                    throw new Error(IFB.errorInstallmentsVsDuration);
                }
            }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex items-center space-x-2">
                            <Switch
                                ...
                                style={{'--provider-color': providerColor} as React.CSSProperties}
                            />
                            <Label htmlFor={`allowConcurrentLoans-${product.id}`}>{IFB.combinOtherFinancing}</Label>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor={`minLoan-${product.id}`}>{IFB.minFinancingAmountLabel}</Label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <Label htmlFor={`maxLoan-${product.id}`}>{IFB.maxFinancingAmountLabel}</Label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <Label htmlFor={`duration-${product.id}`}>{IFB.financingDurationDaysLabel}</Label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <Label htmlFor={`installments-${product.id}`}>{IFB.installmentsLabel}</Label>
                            <Input
                                ...
                                onChange={handleChange}
                                placeholder={IFB.installmentsPlaceholder}
                                disabled={!canEditProduct}
                            />
                        </div>
                        {formData.installments && Number(formData.installments) > 0 && (
                          <div className="space-y-2">
                            <Label>{IFB.settlementIntervalLabel}</Label>
...
function ProvidersTab({ providers, onProvidersChange }: { 
    ...
    const themeColor = useMemo(() => {
        if (currentUser?.role === 'Super Admin' || currentUser?.role === 'Admin') {
            return providers.find(p => p.name === 'NIb Bank')?.colorHex || BRAND_PRIMARY;
        }
        return providers.find(p => p.name === currentUser?.providerName)?.colorHex || BRAND_PRIMARY;
    ...
    if (providers.length === 0) {
        return (
            <>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>{IFB.providersAndProducts}</CardTitle>
                        <CardDescription>
                            {canCreateProvider 
            ...
            <AccordionContent className="p-4 border-t">
              <div className="space-y-6">
                ...
                {canCreateProduct && (
                    ...
                    >
                    <PlusCircle className="mr-2 h-4 w-4" /> {IFB.addNewFinancingProduct}
                    </Button>
                )}
              </div>
            </AccordionContent>
            ...
            {isEnabled && fee.type === 'percentage' && (
                <div className="flex items-center gap-2 pl-[124px]">
                    ...
                    <Select value={fee.calculationBase || 'principal'} onValueChange={(base: 'principal' | 'compound') => onChange({ ...fee, calculationBase: base })}>
                        ...
                        <SelectContent>
                            <SelectItem value="principal">{IFB.financingAmountBaseLabel}</SelectItem>
                            <SelectItem value="compound">{IFB.compoundBaseLabel}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            )}
        ...
        <div className="flex items-center gap-2">
            ...
            <Select value={rule.type} onValueChange={(type: 'fixed' | 'percentageOfPrincipal' | 'percentageOfCompound') => onChange({ ...rule, type })} disabled={!isEnabled}>
                ...
                <SelectContent>
                    ...
                    <SelectItem value="percentageOfPrincipal">{IFB.pctOfFinancingAmount}</SelectItem>
                    <SelectItem value="percentageOfCompound">{IFB.pctOfOutstanding}</SelectItem>
                </SelectContent>
            </Select>
    ...
    const handleSaveTiers = async () => {
        ...
        try {
            const tiersToSend = tiers.map((tier, i) => {
                ...
                if (loanAmount <= 0) {
                    toast({ title: IFB.errorInvalidFinancingAmount, description: `In tier #${i + 1}, the financing amount must be positive.`, variant: 'destructive'});
                    throw new Error(IFB.errorInvalidFinancingAmount);
                }
                 if (product.maxLoan != null && loanAmount > product.maxLoan) {
                    toast({ title: IFB.errorInvalidFinancingAmount, description: `In tier #${i + 1}, the financing amount cannot exceed the product's maximum of ${product.maxLoan}.`, variant: 'destructive'});
                    throw new Error(IFB.errorInvalidFinancingAmount);
                }
            ...
            if (!["Invalid tier data", IFB.errorInvalidFinancingAmount, "Overlapping Tiers"].includes(error.message)) {
                ...
                <CollapsibleTrigger asChild>
                    <CardHeader className="cursor-pointer">
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle>{IFB.financingAmountTiers}</CardTitle>
                                <CardDescription>{IFB.financingAmountTiersDesc}</CardDescription>
                            </div>
                            <ChevronDown className="h-4 w-4 transition-transform duration-200 data-[state=open]:rotate-180" />
                        </div>
                    </CardHeader>
                                ...
                                <Label className="w-16">To Score</Label>
                                 <Input
                                    ...
                                    className="w-28"
                                                disabled={!canEditProduct}
                                />
                                <Label className="w-24">{IFB.financingAmountTierColumn}</Label>
                                 <Input
                                    type="number"
...
function ProductConfiguration({ product, providerColor, onProductUpdate, taxConfig }: { 
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            // Validate financing amount tiers before submitting, in case the user
            ...
            for (let i = 0; i < tiersToValidate.length; i++) {
                ...
                if (config.maxLoan != null && !isNaN(Number(config.maxLoan)) && loanAmount > Number(config.maxLoan)) {
                    toast({ title: IFB.errorInvalidFinancingAmount, description: `In tier #${i + 1}, the financing amount cannot exceed the product's maximum of ${config.maxLoan}.`, variant: 'destructive'});
                    throw new Error(IFB.errorInvalidFinancingAmount);
                }
                if (fromScore > toScore) {
                    toast({ title: 'Invalid Tier', description: `In tier #${i + 1}, the "From Score" cannot be greater than the "To Score".`, variant: 'destructive'});
                 ...
                 <Card className="border-t-0 rounded-t-none">
                    <CardContent className="space-y-4 pt-6">
                        <div className="flex items-center justify-between border-b pb-4">
                            <div className="flex items-center gap-2">
                                <Label htmlFor={`serviceFeeEnabled-${config.id}`} className="font-medium">{IFB.administrationFee}</Label>
                    ...
                    <FeeInput 
                        ...
                        <div className="flex items-center justify-between border-b pb-4 pt-4">
                             <div className="flex items-center gap-2">
                                <Label htmlFor={`dailyFeeEnabled-${config.id}`} className="font-medium">{IFB.profitMargin}</Label>
                        ...
                        <div className="flex items-center justify-between border-b pb-4 pt-4">
                             <div className="flex items-center gap-2">
                                <Label htmlFor={`penaltyRulesEnabled-${config.id}`} className="font-medium">{IFB.lateChargeRules}</Label>
                            </div>
                            <Switch
                            ...
                            {config.installments && Number(config.installments) > 0 && (
                                <div className="flex items-center justify-between mt-3">
                                    <div className="flex items-center gap-2">
                                        <Label htmlFor={`penaltyPerInstallment-${config.id}`} className="font-medium">{IFB.applyLateChargePerInstallment}</Label>
                        ...
                        <div>
                            <div className="space-y-2 p-4 border rounded-md bg-muted/50">
                                ...
                                <Button variant="outline" size="sm" onClick={handleAddPenaltyRule} disabled={!config.penaltyRulesEnabled || !canEditProduct}>
                                    <PlusCircle className="h-4 w-4 mr-2" /> {IFB.addLateChargeRule}
                                </Button>
                            </div>
                        </div>
...
function ConfigurationTab({ providers, onProductUpdate, taxConfig }: { 
    ...
    if (providers.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>{IFB.productFeeConfiguration}</CardTitle>
                    <CardDescription>
...
const TAX_COMPONENTS = [
    { id: 'serviceFee', label: IFB.administrationFee },
    { id: 'interest', label: IFB.profitMargin },
];
    ...
    return (
        <Card>
            <CardHeader className='flex-row items-start justify-between'>
                <div>
                    <CardTitle>{IFB.globalLeviesConfiguration}</CardTitle>
                    <CardDescription>{IFB.taxSettingsReadOnlyDescription}</CardDescription>
                </div>
                <Button asChild variant="outline" size="sm">
                    <Link href="/admin/tax">
                        <Pencil className="mr-2 h-4 w-4" />
                        {IFB.editLeviesConfiguration}
                    </Link>
                </Button>
            </CardHeader>
            <CardContent className="space-y-6">
                <div className="space-y-2">
                    <Label>{IFB.vatRatePercentLabel}</Label>
                    <Input 
                ...
                <div className="space-y-4">
                    <Label>{IFB.leviesAppliedOnComponents}</Label>
...
function LoanCycleTab({ providers, onProviderUpdate }: {
    ...
    if (providers.length === 0) {
        return <Card><CardHeader><CardTitle>{IFB.financingCycleConfiguration}</CardTitle><CardDescription>{IFB.noProvidersForFinancingCycle}</CardDescription></CardHeader></Card>;
    }
    return (
        <Accordion type="multiple" className="w-full space-y-4">
            {providers.map((provider) => (
...
function LoanCycleForm({ product, onUpdate, providerColor }: {
    ...
    const handleSaveLoanCycle = async () => {
        if (loanCycleReadOnly) {
            toast({ title: 'Not authorized', description: IFB.notAuthorizedFinancingCycle, variant: 'destructive' });
            return;
        }
        if (!product.id || !editingMetric) {
            toast({ title: 'Error', description: IFB.productAndMetricRequired, variant: 'destructive' });
            return;
        }
        ...
        try {
            ...
            await postPendingChange({ entityType: 'LoanCycleConfig', entityId: product.id, changeType, payload: JSON.stringify(payload) }, IFB.failedFinancingCycleSubmit);

            // Mark locally as pending (so UI reflects that change was submitted)
            toast({ title: 'Submitted', description: IFB.financingCycleSubmitted });
    ...
    return (
         <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            <CollapsibleTrigger asChild>
                 <button className="flex items-center justify-between w-full space-x-4 px-4 py-2 border rounded-lg bg-background hover:bg-muted/50 transition-colors">
                    <h4 className="text-sm font-semibold">{product.name}</h4>
                     <Badge variant={loanCycleConfig ? 'default' : 'secondary'}>{loanCycleConfig ? IFB.financingCycleConfigured : IFB.financingCycleNotConfigured}</Badge>
                </button>
            </CollapsibleTrigger>
            <CollapsibleContent className="p-4 border rounded-lg bg-background mt-2">
                <div className="flex items-center justify-end">
                    <div className="flex items-center gap-2">
                        <Label>{IFB.enabledLabel}</Label>
                 ...
                 <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div className="space-y-2">
                        <Label>{IFB.progressionMetricLabel}</Label>
                        <Select onValueChange={(v) => setEditingMetric(v as LoanCycleConfig['metric'])} value={editingMetric || undefined}>
                            <SelectTrigger disabled={loanCycleReadOnly}>
                                <SelectValue placeholder={IFB.selectProgressionMetricPlaceholder} />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="TOTAL_COUNT">{IFB.metricTotalFinancingCount}</SelectItem>
                                <SelectItem value="PAID_ON_TIME">{IFB.metricOnTimeSettlements}</SelectItem>
                                <SelectItem value="PAID_EARLY">{IFB.metricEarlySettlements}</SelectItem>
                                <SelectItem value="PAID_LATE">{IFB.metricLateSettlements}</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                </div>
                ...
                <div className="mt-4">
                    ...
                    <div className="mt-4">
                        <Label className="mb-2">{IFB.gradesAndPayouts}</Label>
                ...
                <div className="mt-4 flex justify-end space-x-2">
                    ...
                    <Button type="button" onClick={handleSaveLoanCycle} disabled={isSavingLoanCycle || !editingMetric || loanCycleReadOnly} style={{ backgroundColor: providerColor }} className="text-white">{isSavingLoanCycle ? IFB.saving : IFB.saveFinancingCycle}</Button>
                </div>
            </CollapsibleContent>
        </Collapsible>
    )
                        ...
                        {mounted ? (
                            <Tabs defaultValue="providers" className="space-y-4">
                                <TabsList>
                                    ...
                                    <TabsTrigger value="loanCycles">{IFB.financingCycleTab}</TabsTrigger>
                                ...
                                <TabsContent value="agreement">
                                    <Tabs defaultValue="borrower-agreement" className="space-y-4">
                                        <TabsList>
                                            <TabsTrigger value="borrower-agreement">{IFB.customerAgreementTab}</TabsTrigger>
...
function AgreementTab({ provider, onProviderUpdate }: { provider: LoanProvider, onProviderUpdate: (update: Partial<LoanProvider>) => void }) {
    ...
    const handleSave = async () => {
        ...
        if (!terms || !terms.content.trim()) {
            toast({ title: "Error", description: IFB.shariaTermsEmptyError, variant: "destructive" });
            return;
        }
        setIsLoading(true);
        try {
...
function DeliveryAgreementTab({ providerId }: { providerId: string }) {
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">{IFB.shariaAgreementBeforeDeliveryNote}</p>
            {pendingApproval && (
            ...
            <UIDialogContent className="sm:max-w-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 py-4">
                    <div>
                        ...
                        <Input id="data-type-name" value={name} onChange={e => setName(e.target.value)} placeholder={IFB.externalRiskDataExample} required disabled={!!readOnly} />
                    </div>
