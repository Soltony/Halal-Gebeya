...
export function ApplyClient({
  ...
  const eligibilityResult = useMemo(() => {
    ...
    return {
      ...
      reason: IFB.eligibleForFinancing,
    };
    ...
    try {
      ...
      if (isBnplOrder) {
        ...
        const displayLoan: LoanDetails = {
          ...
          installments: (details as any).installments,
        };
        ...
        toast({
          title: "Order placed!",
          description:
            IFB.orderPlacedFinancingAfterDelivery,
        });
      } else {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || IFB.saveFinancingFailed);
        }
        ...
        setLoanDetails(displayLoan);
        setStep("details");
        toast({ title: "Success!", description: IFB.financingSavedSuccess });
            ...
            if (!disRes.ok) {
              ...
              toast({
                ...
                description:
                  err?.error ||
                  JSON.stringify(err) ||
                  IFB.upstreamFinancingReleaseFailed,
                variant: "destructive",
              });
            } else {
              toast({
                title: "Disbursement sent",
                description: IFB.externalFinancingReleaseSent,
              });
            }
          } else {
            toast({
              ...
              description:
                IFB.noFinancingReleaseAccountSelected,
              variant: "warning",
            });
          }
        } catch (err: any) {
