...
const formatPenaltyRule = (rule: PenaltyRule | undefined, type: 'summary' | 'full' = 'full'): string => {
    ...
    if (rule.type === 'percentageOfPrincipal') {
        valueString += ' of financing amount';
...
const taxComponentLabels: Record<string, string> = {
    serviceFee: IFB.administrationFee,
    interest: IFB.profitMargin,
};
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            {activeInstallment && (
                                <p className="text-sm text-muted-foreground">
                                    ...
                                    {instPenalty > 0 && (
                                        <span className="ml-2">• {IFB.lateCharge}: {formatCurrency(instPenalty)} ETB</span>
                                    )}
                                </p>
                            )}
                        </div>
                        <div className="text-right">
                             ...
                             {activeInstallment && instOutstanding !== null && (
                                <p className="text-xs text-muted-foreground">{IFB.financingAmount}: {formatCurrency(instOutstanding)} ETB</p>
                             )}
                        </div>
                    </div>
                     <div className="flex justify-end mt-2">
                        <Button onClick={() => onRepay(activeLoan, balanceDue)} style={{ backgroundColor: providerColor }} className="text-white">{IFB.settle}</Button>
                    </div>
                </CardContent>
            </Card>
        );
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                                {IFB.financingLimit} {formatCurrency(product.minLoan ?? 0)} to {formatCurrency((product.maxLoan && product.maxLoan > 0) ? product.maxLoan : (product.availableLimit ?? 0))}
                            </p>
                        </div>
                    </div>
                    {isEligible ? (
