...
const DashboardView = ({ data, color }: { data: DashboardData, color: string }) => {
    ...
    const loanStatusData = useMemo(() => [
      ...
      { name: IFB.statusActiveOutstanding, value: rawLoanStatusData.find(d => d.name === 'Active (Unpaid)')?.value || 0, color: `${color}B3` }, // 70% opacity
      { name: IFB.statusPastDue, value: rawLoanStatusData.find(d => d.name === 'Overdue')?.value || 0, color: `${color}66` }, // 40% opacity
    ...
    return (
      <div className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.poolStartingCapital}</CardTitle>
                    <Wallet className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{formatCurrency(initialFund)}</div>
                    <p className="text-xs text-muted-foreground">{IFB.poolStartingCapitalHint}</p>
                </CardContent>
            </Card>
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.availablePoolBalance}</CardTitle>
                    <Wallet className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{formatCurrency(providerFund)}</div>
                    <p className="text-xs text-muted-foreground">Balance available for new financing releases</p>
                </CardContent>
            </Card>
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.totalFinancingReleased}</CardTitle>
                    <TrendingUp className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{formatCurrency(totalDisbursed)}</div>
                    <p className="text-xs text-muted-foreground">All-time total financing released</p>
                </CardContent>
            </Card>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.dailyFinancingReleased}</CardTitle>
                ...
                <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">Total financing released today</p>
                </CardContent>
            </Card>
             <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.dailySettlements}</CardTitle>
                    <ArrowDownWideNarrow className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{formatCurrency(dailyRepayments)}</div>
                    <p className="text-xs text-muted-foreground">Total settled today</p>
                </CardContent>
            </Card>
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.totalFinancingCount}</CardTitle>
                    <Landmark className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{totalLoans}</div>
                    <p className="text-xs text-muted-foreground">All-time financing facilities</p>
                </CardContent>
            </Card>
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">{IFB.customers}</CardTitle>
                    <Users className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{totalUsers}</div>
                    <p className="text-xs text-muted-foreground">{IFB.uniqueCustomersWithFinancing}</p>
                </CardContent>
            </Card>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            <Card className="lg:col-span-4">
                <CardHeader>
                    <CardTitle>{IFB.financingReleasedOverTime}</CardTitle>
            ...
            <Card className="lg:col-span-3">
                <CardHeader>
                    <CardTitle>{IFB.financingStatusDistribution}</CardTitle>
                    <CardDescription>{IFB.financingStatusDistributionDesc}</CardDescription>
                </CardHeader>
                <CardContent>
                   <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
                ...
                <CardContent>
                    <LedgerDetailRow title={IFB.financingAmountReceivable} value={receivables.principal} />
                    <LedgerDetailRow title={IFB.profitReceivable} value={receivables.interest} />
                    <LedgerDetailRow title={IFB.administrationFeeReceivable} value={receivables.serviceFee} />
                    <LedgerDetailRow title={IFB.lateChargeReceivable} value={receivables.penalty} />
            ...
            <Card>
                ...
                <CardContent>
                    <LedgerDetailRow title={IFB.financingAmountReceived} value={collections.principal} />
                    <LedgerDetailRow title={IFB.profitReceived} value={collections.interest} />
                    <LedgerDetailRow title={IFB.administrationFeeReceived} value={collections.serviceFee} />
                    <LedgerDetailRow title={IFB.lateChargeReceived} value={collections.penalty} />
        ...
        <div className="grid gap-4 mt-4">
            <Card className="col-span-full">
              <CardHeader>
                <CardTitle>{IFB.financingProductsOverview}</CardTitle>
                <CardDescription>
                  {IFB.financingProductsOverviewDesc}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
